...
def borrar_curso_de_carrera(json):
    return str(db.carrera.update_one({'_id': ObjectId(json['id_carrera'])},{'$pull': {'cursos': {'_id': ObjectId(json['id_carrera'])}}}).modified_count)

# -----------------Cursos-------------------------


def crear_curso(json):
    return str(db.cursos.insert_one(json).insert_id)
...
def actualizar_curso(curso):
   ...
   #Tambien se puede crear una funcion y luego actualizar
...
#Consulta con el índice de nombre
